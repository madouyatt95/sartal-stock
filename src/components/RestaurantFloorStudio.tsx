  ...
  return <section className={`restaurant-floor-studio ${compact ? 'compact' : ''} ${studioMode ? 'editing' : ''} ${tabletMode ? 'tablet-mode' : ''} ${heatmap ? 'heatmap-mode' : ''}`}>
    <header className="restaurant-floor-toolbar">
      ...
      <div className="restaurant-floor-toolbar-actions">
        ...
        <button className={heatmap ? 'active secondary' : 'secondary'} onClick={() => setHeatmap(value => !value)}><ScanLine size={18} /> {heatmap ? 'Vue normale' : 'Heatmap'}</button>
        <button className={tabletMode ? 'active secondary' : 'secondary'} onClick={() => setTabletMode(value => !value)}>{tabletMode ? <Maximize2 size={18} /> : <Tablet size={18} />}{tabletMode ? 'Quitter tablette' : 'Mode tablette'}</button>
        {editable && <button className={studioMode ? 'active' : ''} onClick={() => studioMode ? leaveStudio() : enterStudio()}>{studioMode ? <X size={18} /> : <PencilRuler size={18} />}{studioMode ? 'Quitter le Studio' : 'Ouvrir le Studio'}</button>}
      </div>
    </header>

    <div className="restaurant-floor-levels" aria-label="Niveaux du restaurant">{floors.map(floor => <button className={selectedFloor === floor ? 'active' : ''} key={floor} onClick={() => { setSelectedFloor(floor); setSelectedObject(undefined); }}><Layers size={15} /> {floor}</button>)}</div>
    <div className="restaurant-floor-zones" aria-label="Zones du restaurant">{zones.map(zone => <button className={selectedZone === zone ? 'active' : ''} key={zone} onClick={() => { setSelectedZone(zone); setSelectedObject(undefined); }}><MapPin size={14} /> {zone}</button>)}</div>

    {studioMode && <section className="restaurant-studio-commandbar">
      <div className="restaurant-studio-command-group"><button disabled={!undoStack.length} onClick={undo} title="Annuler"><Undo2 size={18} /></button><button disabled={!redoStack.length} onClick={redo} title="Rétablir"><Redo2 size={18} /></button><button onClick={() => setZoom(value => clamp(value - 0.1, 0.65, 1.7))} title="Dézoomer"><ZoomOut size={18} /></button><b>{Math.round(zoom * 100)}%</b><button onClick={() => setZoom(value => clamp(value + 0.1, 0.65, 1.7))} title="Zoomer"><ZoomIn size={18} /></button><button className={panMode ? 'active' : ''} onClick={() => setPanMode(value => !value)}><Move size={17} /> Déplacer</button><button onClick={() => { setZoom(1); setPan({ x: 0, y: 0 }); }}><Maximize2 size={17} /> Recentrer</button></div>
      <div className="restaurant-studio-command-group"><button className={draftSettings.showGrid ? 'active' : ''} onClick={() => mutateDraft(snapshot => { snapshot.settings.showGrid = !snapshot.settings.showGrid; })}><Grid3X3 size={17} /> Grille</button><button className={draftSettings.snapToGrid ? 'active' : ''} onClick={() => mutateDraft(snapshot => { snapshot.settings.snapToGrid = !snapshot.settings.snapToGrid; })}><MousePointer2 size={17} /> Magnétisme</button><button className={multiSelect ? 'active' : ''} onClick={() => { setMultiSelect(value => !value); setSelectedTableIds(new Set()); }}><Columns3 size={17} /> Multi-sélection</button></div>
      <div className="restaurant-studio-publish"><input value={draftLabel} onChange={event => setDraftLabel(event.target.value)} aria-label="Nom du plan" /><button onClick={saveDraft}><Save size={17} /> Brouillon</button><button className="publish" onClick={publish}><CheckCircle2 size={17} /> Publier</button></div>
    </section>}

    {studioMode && selectedTableIds.size > 0 && <section className="restaurant-studio-selectionbar"><span><strong>{selectedTableIds.size}</strong> table(s) sélectionnée(s)</span><button onClick={() => alignSelection('center')}><AlignCenter size={16} /> Aligner ligne</button><button onClick={() => alignSelection('column')}><Columns3 size={16} /> Aligner colonne</button><button onClick={() => alignSelection('distribute')}><ArrowLeftRight size={16} /> Distribuer</button><button onClick={duplicateSelection}><Copy size={16} /> Dupliquer</button><button disabled={selectedTableIds.size < 2} onClick={mergeSelection}><Merge size={16} /> Fusionner</button><button disabled={selectedTableIds.size !== 1 || !draftTables.find(table => selectedTableIds.has(table.id))?.mergedFrom?.length} onClick={splitSelection}><Split size={16} /> Séparer</button><button className="danger" onClick={removeSelection}><Trash2 size={16} /> Retirer</button><button onClick={() => { setSelectedTableIds(new Set()); setSelectedObject(undefined); }}>Désélectionner</button></section>}
    ...
    <div className={`restaurant-floor-workspace ${studioMode || selectedRuntime || orderPanelOrderId ? 'has-panel' : ''} ${orderPanelOrderId ? 'has-order-panel' : ''}`}>
      ...
      {!studioMode && orderPanelOrderId && selectedRuntime?.order?.id === orderPanelOrderId ? <RestaurantTableOrderPanel state={state} orderId={orderPanelOrderId} tableLabel={selectedRuntime.table.label} capacity={selectedRuntime.table.capacity} actor={actor} onClose={() => setOrderPanelOrderId('')} /> : studioMode && tableEditor ? <aside className="restaurant-studio-editor">
        <header><div><span>TABLE SÉLECTIONNÉE</span><h3>{tableEditor.label}</h3></div><button onClick={() => setSelectedObject(undefined)} aria-label="Fermer"><X size={18} /></button></header>
        ...
        <fieldset><legend>Forme</legend><div>{(Object.entries(SHAPE_LABELS) as Array<[RestaurantDiningTableShape, string]>).map(([shape, label]) => <button className={tableEditor.shape === shape ? 'active' : ''} key={shape} onClick={() => setTableEditor({ ...tableEditor, shape })}><i className={shape} /> {label}</button>)}</div></fieldset>
        ...
        <div className="restaurant-studio-editor-actions"><button className="danger" onClick={removeSelection}><Trash2 size={17} /> Retirer</button><button className="save" onClick={saveTableEditor}><CheckCircle2 size={17} /> Appliquer</button></div>
      </aside> : studioMode && elementEditor ? <aside className="restaurant-studio-editor">
        <header><div><span>ÉLÉMENT ARCHITECTURAL</span><h3>{elementEditor.label}</h3></div><button onClick={() => setSelectedObject(undefined)}><X size={18} /></button></header>
        ...
        <div className="restaurant-studio-editor-actions"><button className="danger" onClick={removeSelection}><Trash2 size={17} /> Retirer</button><button className="save" onClick={saveElementEditor}><CheckCircle2 size={17} /> Appliquer</button></div>
        ...
        <button className="restaurant-studio-add-table" onClick={addTable}><Plus size={18} /> Ajouter une table</button>
        <div className="restaurant-studio-elements">{ELEMENT_CATALOG.map(item => <button key={item.type} onClick={() => addElement(item)}><i className={item.type} /><span>{item.label}</span></button>)}</div>
        <section className="restaurant-studio-background"><header><FileImage size={18} /><div><strong>Fond de plan</strong><small>{currentBackground?.name || 'Aucun plan importé'}</small></div></header><input ref={backgroundInputRef} type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={event => importBackground(event.target.files?.[0])} /><button onClick={() => backgroundInputRef.current?.click()}><Upload size={16} /> Importer une image</button>{currentBackground && <><label>Opacité <b>{Math.round(currentBackground.opacity * 100)}%</b><input type="range" min="0.1" max="0.9" step="0.05" value={currentBackground.opacity} onChange={event => updateBackgroundOpacity(Number(event.target.value))} /></label><button className="danger" onClick={removeBackground}><Trash2 size={15} /> Retirer le fond</button></>}</section>
        ...
        <header><div><span>{STATUS_LABELS[selectedRuntime.status]}</span><h3>{selectedRuntime.table.label}</h3><p>{selectedRuntime.table.capacity} places · {selectedRuntime.table.zone}</p></div><button onClick={() => setSelectedObject(undefined)}><X size={18} /></button></header>
        ...
        {selectedRuntime.order?.status === 'served' && <><div className="restaurant-floor-bill"><ReceiptText size={17} /><span><small>Addition en attente</small><strong>{formatFCFA(Math.max(0, selectedRuntime.order.total - selectedRuntime.order.payments.reduce((sum, payment) => sum + payment.amount, 0)))}</strong></span></div><button className="restaurant-floor-primary" onClick={() => execute(() => state.requestRestaurantTableBill(selectedRuntime.table.id, actor), 'Demande d’addition transmise à la caisse.')}><ReceiptText size={16} /> Demander l’addition</button></>}
        {selectedRuntime.customer && <button className="restaurant-floor-secondary" onClick={() => createTableQR(selectedRuntime.table.id)}><QrCode size={16} /> Créer l’accès QR client</button>}
        ...
        {!['free', 'reserved'].includes(selectedRuntime.status) && <div className="restaurant-floor-transfer"><select value={transferTargetId} onChange={event => setTransferTargetId(event.target.value)}><option value="">Transférer vers une table libre</option>{freeTransferTargets.filter(item => item.table.capacity >= (selectedRuntime.reservation?.guests || 1)).map(item => <option key={item.table.id} value={item.table.id}>{item.table.label} · {item.table.capacity} places</option>)}</select><button disabled={!transferTargetId} onClick={() => { if (execute(() => state.transferRestaurantTable(selectedRuntime.table.id, transferTargetId, actor), 'Table transférée avec sa commande et sa réservation.')) setTransferTargetId(''); }}><ArrowLeftRight size={16} /> Transférer</button></div>}
      </aside> : null}
    </div>

    <footer className="restaurant-floor-legend"><div>{(Object.entries(STATUS_LABELS) as Array<[FloorStatus, string]>).map(([status, label]) => <span className={status} key={status}><i /> {label}</span>)}</div><div className="restaurant-floor-waiter-legend">{waiters.map((waiter, index) => <span key={waiter.id}><i style={{ background: WAITER_COLORS[index % WAITER_COLORS.length] }} /> {waiter.name.split(' ')[0]} <b>{activeCountForWaiter(waiter.id)}</b></span>)}</div>{editable && <button onClick={() => execute(() => state.rebalanceRestaurantServiceSections(posId, actor), 'Secteurs rééquilibrés selon l’équipe disponible.')}><Users size={16} /> Rééquilibrer les secteurs</button>}<button onClick={() => setShowGovernance(value => !value)}><History size={16} /> Versions & journal</button><b>{visibleTables.length} table(s)</b></footer>

    {showGovernance && <section className="restaurant-floor-governance"><header><div><span>GOUVERNANCE DU PLAN</span><h3>Versions publiées et traçabilité</h3></div><button onClick={() => setShowGovernance(false)}><X size={18} /></button></header><div><section><h4><History size={17} /> Versions</h4>{versions.map(version => <article key={version.id}><div><span className={version.status}>{version.status === 'published' ? 'Publié' : version.status === 'draft' ? 'Brouillon' : 'Archivé'}</span><strong>{version.label}</strong><small>{version.createdBy} · {new Date(version.createdAt).toLocaleString('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</small></div><footer>{version.status === 'draft' && <button onClick={() => enterStudio(version)}><PencilRuler size={14} /> Ouvrir</button>}{editable && version.status !== 'published' && <button onClick={() => restoreVersion(version)}><History size={14} /> Restaurer</button>}{version.status === 'draft' && <button className="danger" onClick={() => execute(() => state.deleteRestaurantFloorPlanVersion(version.id), 'Brouillon supprimé.')}><Trash2 size={14} /></button>}</footer></article>)}</section><section><h4><ScanLine size={17} /> Journal d’audit</h4>{auditEntries.map(entry => <article key={entry.id}><div><span>{entry.action.replaceAll('_', ' ')}</span><strong>{entry.summary}</strong><small>{entry.actor} · {new Date(entry.createdAt).toLocaleString('fr-FR', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit' })}</small></div></article>)}{auditEntries.length === 0 && <p>Aucune modification enregistrée.</p>}</section></div></section>}

    {confirmExit && <div className="restaurant-floor-confirm"><section><h3>Quitter sans publier ?</h3><p>Les modifications locales non enregistrées seront perdues. Le plan actuellement publié restera inchangé.</p><div><button onClick={() => setConfirmExit(false)}>Continuer le Studio</button><button className="danger" onClick={forceLeaveStudio}>Quitter sans enregistrer</button></div></section></div>}
