...
export const RestaurantFloorStudio: React.FC<RestaurantFloorStudioProps> = ({ state, posId, editable = false, compact = false, operatorName }) => {
  ...
  const unassignedReservations = db.restaurantReservations.filter(item => item.posId === posId && item.date === today && item.status === 'confirmed' && !item.tableNumber);
  ...
  return <section className={`restaurant-floor-studio ${compact ? 'compact' : ''} ${studioMode ? 'editing' : ''} ${tabletMode ? 'tablet-mode' : ''} ${heatmap ? 'heatmap-mode' : ''}`}>
    ...
    <div className={`restaurant-floor-workspace ${studioMode || selectedRuntime ? 'has-panel' : ''}`}>
      ...
      {studioMode && tableEditor ? <aside className="restaurant-studio-editor">
        ...
        {selectedRuntime.status === 'free' && <div className="restaurant-floor-live-actions"><button onClick={() => setWalkInOpen(value => !value)}><UserPlus size={16} /> Installer sans réservation</button>{walkInOpen && <div className="restaurant-floor-inline-form"><input value={walkIn.name} onChange={event => setWalkIn({ ...walkIn, name: event.target.value })} placeholder="Nom du client" /><input type="number" min="1" max={selectedRuntime.table.capacity} value={walkIn.guests} onChange={event => setWalkIn({ ...walkIn, guests: Number(event.target.value) || 1 })} /><button onClick={() => runWalkIn(selectedRuntime.table.id)}>Installer</button></div>}{unassignedReservations.length > 0 && <><select value={reservationChoice} onChange={event => setReservationChoice(event.target.value)}><option value="">Choisir une réservation en attente</option>{unassignedReservations.map(reservation => <option key={reservation.id} value={reservation.id}>{reservation.time} · {db.sartalCustomers.find(customer => customer.id === reservation.customerId)?.fullName} · {reservation.guests} pers.</option>)}</select><button disabled={!reservationChoice} onClick={() => { if (execute(() => state.seatRestaurantReservation(reservationChoice, selectedRuntime.table.id, actor), 'Réservation attribuée et client installé.')) setReservationChoice(''); }}><DoorOpen size={16} /> Attribuer et installer</button></>}</div>}
        ...
        {selectedRuntime.status === 'occupied' && !selectedRuntime.order && <button className="restaurant-floor-primary" onClick={() => execute(() => state.openRestaurantTableOrder(selectedRuntime.table.id, actor), 'Commande ouverte et visible par la salle.')}><ReceiptText size={16} /> Ouvrir la commande</button>}
