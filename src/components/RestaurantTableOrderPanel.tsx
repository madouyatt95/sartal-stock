...
export const RestaurantTableOrderPanel: React.FC<RestaurantTableOrderPanelProps> = ({ state, orderId, tableLabel, capacity, actor, onClose }) => {
  ...
  return <aside className="restaurant-order-panel" aria-label={`Commande ${tableLabel}`}>
    <header className="restaurant-order-panel-header"><div><span>PRISE DE COMMANDE</span><h2>Table {tableLabel}</h2><p>{order.items.filter(item => item.status !== 'voided').length} ligne(s) · sortie {warehouse?.name || 'dépôt du POS'}</p></div><button type="button" onClick={onClose} aria-label="Fermer la commande"><X size={21} /></button></header>
    ...
    <div className="restaurant-order-menu">{visibleMenu.map(item => <button type="button" key={item.product!.id} onClick={() => addProduct(item.product!.id)}><span><strong>{item.product!.name}</strong><small>{item.product!.category} · {COURSE_LABELS[course]} · convive {seatNumber || 'table'}</small></span><b>{formatFCFA(item.pricing.salePrice)}</b><Plus size={17} /></button>)}{visibleMenu.length === 0 && <p>Aucun article disponible pour ce filtre.</p>}</div>
