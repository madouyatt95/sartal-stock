...
export const RestaurantTableServiceModal: React.FC<RestaurantTableServiceModalProps> = ({ state, posId, tableId, editable = false, operatorId, operatorName, initialTab = 'summary', onClose, onTableChange }) => {
  ...
  const tableRequests = activeOrder ? db.sartalServiceRequests.filter(item => item.referenceId === activeOrder.id && !['completed', 'cancelled'].includes(item.status)) : [];
  const request = tableRequests.find(item => item.type === 'cash_payment') || tableRequests[0];
  ...
  const waitingForPassCount = activeOrder?.items.filter(item => item.status === 'ready' && ['kitchen', 'dessert'].includes(item.station || 'kitchen') && !item.passedAt).length || 0;
  const canConfirmService = activeOrder?.status === 'ready' && waitingForPassCount === 0;
    ...
    <aside className={`restaurant-table-service-modal restaurant-table-modal-shell ${status}`} role="dialog" aria-modal="true" aria-label={`Actions table ${table.label}`} onMouseDown={event => event.stopPropagation()}>
      ...
      <div className="restaurant-table-service-content">
        {tab === 'summary' && <section className="restaurant-table-summary">
          ...
          {request && <button type="button" className="restaurant-floor-primary" disabled={request.type === 'cash_payment' && request.status === 'accepted' && !canCollect} onClick={() => execute('request', () => request.status === 'requested'
            ? state.updateSartalServiceRequest(request.id, 'accepted', operatorName)
            : request.type === 'cash_payment' && operatorId
              ? state.confirmRestaurantCashPayment(request.id, operatorId)
              : state.updateSartalServiceRequest(request.id, 'completed', operatorName), request.type === 'cash_payment' && request.status === 'accepted' ? 'Espèces confirmées et addition mise à jour.' : 'Demande client mise à jour.')}><BellRing size={16} /> {request.status === 'requested' ? 'Prendre la demande' : request.type === 'cash_payment' ? canCollect ? `Confirmer ${formatFCFA(request.requestedAmount || 0)} en espèces` : 'Confirmation caisse requise' : 'Confirmer effectué'}</button>}
          {!request && canConfirmService && <button type="button" className="restaurant-floor-primary" onClick={() => execute('serve', () => state.updateRestaurantGuestOrderStatus(activeOrder!.id, 'served'), `${table.label} marquée servie.`)}><CheckCircle2 size={16} /> Confirmer le service</button>}
          {!request && activeOrder?.status === 'ready' && waitingForPassCount > 0 && <div className="restaurant-floor-pass-status"><Clock3 size={18} /><span><strong>Contrôle du passe en attente</strong><small>{waitingForPassCount} article(s) doivent être contrôlés avant le départ en salle.</small></span><button type="button" onClick={() => setTab('order')}>Voir le ticket</button></div>}
