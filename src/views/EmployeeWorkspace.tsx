...
const ROLE_CONFIG: Record<EmployeeRole, RoleConfig> = {
  ...
  housekeeper: { label: 'Agent d’étage', team: 'Étages', actionLabel: 'Chambre', icon: Sparkles, color: '#9a4678' },
  housekeeping_manager: { label: 'Gouvernante', team: 'Contrôle chambres', actionLabel: 'Contrôler', icon: ClipboardCheck, color: '#7e3f78' },
  ...
  dispatcher: { label: 'Responsable dispatch', team: 'Dispatch', actionLabel: 'Tournées', icon: Route, color: '#315f98' },
  driver: { label: 'Livreur', team: 'Livraison', actionLabel: 'Livraison', icon: Truck, color: '#326894' },
  maintenance: { label: 'Technicien maintenance', team: 'Maintenance', actionLabel: 'Intervenir', icon: Wrench, color: '#7b5a32' },
...
const employeeScheduleMinutes = (schedule: { startTime: string; endTime: string }) => {
...
export const EmployeeWorkspace: React.FC<EmployeeWorkspaceProps> = ({ state, initialRole, demoAutoStart = false }) => {
  ...
  const [waiterPaymentOrderId, setWaiterPaymentOrderId] = useState('');
  ...
  const [receptionPaymentMethod, setReceptionPaymentMethod] = useState<PaymentType>('wave');
  const [receptionPaymentAmount, setReceptionPaymentAmount] = useState('');
  const [receptionPaymentReference, setReceptionPaymentReference] = useState('');
  ...
  const [selectedDispatchDriverId, setSelectedDispatchDriverId] = useState('');
  ...
  const employeePlanningDays = Array.from({ length: 7 }, (_, index) => addDaysToKey(employeePlanningWeek, index));
  const employeeWeekSchedules = employeeSchedules.filter(item => item.date >= employeePlanningDays[0] && item.date <= employeePlanningDays[6]);
  ...
  const roleAssignments = useMemo(() => {
    ...
    if (['storekeeper', 'picker', 'dispatcher', 'driver'].includes(employee.role)) {
      return db.warehouses
        .filter(item => item.id === employee.warehouseId)
        .map(item => ({ id: item.id, label: item.name }));
    }
    return db.sites.filter(item => item.id === employee.siteId).map(item => ({ id: item.id, label: ['receptionist', 'housekeeper', 'housekeeping_manager', 'maintenance'].includes(employee.role) ? `${item.name} · Hôtel / PMS` : item.name }));
  ...
  const handoverRoleFallback: Partial<Record<EmployeeRole, EmployeeRole[]>> = {
    ...
    housekeeping_manager: ['housekeeping_manager', 'housekeeper'],
    maintenance: ['maintenance', 'receptionist'],
    dispatcher: ['dispatcher', 'picker', 'driver'],
  ...
  const getRoleTasks = (): StaffTask[] => {
    ...
    if (['housekeeper', 'housekeeping_manager'].includes(employee.role)) {
      db.pmsHousekeepingTasks.filter(item => employee.role === 'housekeeping_manager' ? item.status === 'completed' : !['completed', 'inspected'].includes(item.status)).forEach(item => {
        const room = db.pmsRooms.find(roomItem => roomItem.id === item.roomId);
        const nextStatus = employee.role === 'housekeeping_manager' ? 'inspected' : item.status === 'pending' ? 'in_progress' : 'completed';
    ...
    if (employee.role === 'dispatcher') {
      db.deliveryOrders.filter(item => ['ready', 'out_for_delivery', 'failed'].includes(item.status)).forEach(item => tasks.push({
        id: item.id,
        title: item.status === 'failed' ? `Incident · ${item.customerName}` : `${item.id} · ${item.zone || 'Zone à confirmer'}`,
        detail: item.status === 'failed' ? item.deliveryIssue || 'Incident à qualifier' : `${item.customerName} · ${item.deliverySlotLabel || 'Dès que possible'}`,
        meta: item.driverName ? `${item.driverName} · ${ORDER_STATUS[item.status]}` : 'Livreur à affecter',
        tone: item.status === 'failed' || !item.driverName ? 'urgent' : item.status === 'out_for_delivery' ? 'active' : 'waiting',
        actionLabel: 'Piloter',
        action: () => { setSelectedDeliveryId(item.id); setTab('action'); }
      }));
    }
    ...
    if (employee.role === 'maintenance') {
      db.pmsMaintenanceTickets.filter(item => !['verified'].includes(item.status)).forEach(item => {
        const room = db.pmsRooms.find(roomItem => roomItem.id === item.roomId);
        tasks.push({ id: item.id, title: `${room ? `Chambre ${room.roomNumber}` : item.roomId} · ${item.equipment}`, detail: item.note, meta: `${item.priority === 'critical' ? 'CRITIQUE · ' : ''}${ORDER_STATUS[item.status] || item.status} · ${item.assignedTo}`, tone: item.priority === 'critical' || item.priority === 'urgent' ? 'urgent' : item.status === 'in_progress' ? 'active' : 'waiting', actionLabel: 'Intervenir', action: () => { setSelectedMaintenanceId(item.id); setMaintenanceCost(String(item.actualCost || item.estimatedCost || '')); setMaintenanceNote(item.note); setTab('action'); } });
      });
    }
  ...
  const metrics = (() => {
    ...
    const roleMetrics: Record<EmployeeRole, Array<{ label: string; value: string | number; tone?: string }>> = {
      ...
      cashier: [
        { label: 'Caisse', value: db.cashSessions.some(item => item.posId === (assignedPosId || employee.posId) && item.status === 'open') ? 'Ouverte' : 'Fermée', tone: activeShift ? 'success' : 'alert' },
      ...
      housekeeper: [
        ...
        { label: 'Terminées', value: db.pmsHousekeepingTasks.filter(item => item.status === 'completed').length },
      ...
      housekeeping_manager: [
        { label: 'À contrôler', value: db.pmsHousekeepingTasks.filter(item => item.status === 'completed').length, tone: 'alert' },
        { label: 'Contrôlées', value: db.pmsHousekeepingTasks.filter(item => item.status === 'inspected').length, tone: 'success' },
        { label: 'En nettoyage', value: db.pmsHousekeepingTasks.filter(item => item.status === 'in_progress').length },
        { label: 'Anomalies linge', value: db.pmsHousekeepingTasks.filter(item => item.linenStatus === 'missing').length, tone: 'alert' }
      ],
      storekeeper: [
      ...
      dispatcher: [
        { label: 'Prêtes à partir', value: db.deliveryOrders.filter(item => item.status === 'ready').length },
        { label: 'En tournée', value: db.deliveryOrders.filter(item => item.status === 'out_for_delivery').length },
        { label: 'Sans livreur', value: db.deliveryOrders.filter(item => ['ready', 'out_for_delivery'].includes(item.status) && !item.driverName).length, tone: 'alert' },
        { label: 'Incidents', value: db.deliveryOrders.filter(item => item.status === 'failed').length, tone: 'alert' }
      ],
      driver: [
      ...
      maintenance: [
        { label: 'Tickets ouverts', value: db.pmsMaintenanceTickets.filter(item => item.status === 'open').length },
        { label: 'En intervention', value: db.pmsMaintenanceTickets.filter(item => item.status === 'in_progress').length },
        { label: 'Critiques', value: db.pmsMaintenanceTickets.filter(item => item.priority === 'critical' && !['resolved', 'verified'].includes(item.status)).length, tone: 'alert' },
        { label: 'À faire vérifier', value: db.pmsMaintenanceTickets.filter(item => item.status === 'resolved').length, tone: 'success' }
      ],
      customer_experience: [
    ...
    <section className="staff-shift-start" style={{ '--staff-role': roleConfig.color } as React.CSSProperties}>
      ...
      <div className="staff-shift-grid">
        <section className="staff-start-form">
          ...
          <button className="staff-pre-shift-planning-toggle" onClick={() => setPreShiftPlanningOpen(value => !value)}><CalendarCheck size={17} /> {preShiftPlanningOpen ? 'Masquer mes prochains services' : 'Voir tout mon planning avant de commencer'} <ChevronRight size={16} /></button>
          {preShiftPlanningOpen && <div className="staff-pre-shift-planning">{employeeSchedules.filter(schedule => schedule.date >= new Date().toISOString().slice(0, 10)).slice(0, 6).map(schedule => <article key={schedule.id}><time><strong>{new Date(`${schedule.date}T12:00:00`).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'short' })}</strong><span>{schedule.startTime}–{schedule.endTime}</span></time><div><strong>{schedule.assignmentLabel}</strong><small>{SCHEDULE_STATUS[schedule.status]}</small></div></article>)}{employeeSchedules.filter(schedule => schedule.date >= new Date().toISOString().slice(0, 10)).length === 0 && <p>Aucun service planifié pour le moment.</p>}</div>}
  ...
  const renderToday = () => (
    <div className="staff-today">
      ...
      <section className="staff-wellbeing-strip">
        ...
        <article><CalendarCheck size={20} /><span><small>Mon planning</small><strong>{nextSchedule ? `${new Date(`${nextSchedule.date}T12:00:00`).toLocaleDateString('fr-FR', { weekday: 'short', day: 'numeric', month: 'short' })} · ${nextSchedule.startTime}-${nextSchedule.endTime}` : 'Aucun service à venir'}</strong></span><button onClick={() => setTab('schedule')}>Voir</button></article>
      </section>
  ...
  const renderWaiterAction = () => {
    ...
    return <section className="staff-action-layout">
      <div className="staff-action-main staff-floor-main"><RestaurantFloorStudio state={state} posId={assignedPosId || employee.posId || 'pos-1'} compact operatorId={employee.id} operatorName={employee.name} paymentOrderRequestId={waiterPaymentOrderId} onPaymentOrderRequestHandled={() => setWaiterPaymentOrderId('')} /></div>
      <aside className="staff-action-side"><header><ReceiptText size={19} /><div><strong>Prochaine addition</strong><small>Table servie</small></div></header>{orders.filter(item => item.status === 'served').slice(0, 2).map(order => <article key={order.id}><span>Table {order.tableNumber}</span><strong>{formatFCFA(order.total - order.payments.reduce((sum, payment) => sum + payment.amount, 0))}</strong><small>{order.roomNumber ? `Imputable chambre ${order.roomNumber}` : 'Paiement sur place'}</small>{canEmployee('table_payment') ? <button type="button" onClick={() => setWaiterPaymentOrderId(order.id)}>Encaisser à table</button> : <button type="button" onClick={() => { const table = db.restaurantDiningTables.find(item => item.posId === order.posId && item.label === order.tableNumber); if (table) execute(() => state.requestRestaurantTableBill(table.id, employee.name), 'Addition transmise à la caisse.'); }}>Transmettre à la caisse</button>}</article>)}<button type="button" className="staff-secondary-link" onClick={() => setTab('tasks')}>Voir toutes les tâches de salle <ArrowRight size={15} /></button></aside>
    </section>;
  };

  const renderCashierAction = () => {
    const cashSession = db.cashSessions.find(item => item.posId === (assignedPosId || employee.posId) && item.status === 'open');
    ...
    const isOrderLineAllocated = (line: { id?: string; seatNumber?: number }) => Boolean(selectedOrder?.payments.some(payment => (line.id && payment.itemIds?.includes(line.id)) || (line.seatNumber && payment.seatNumbers?.includes(line.seatNumber))));
      ...
      <div className="staff-checkout-panel">
        ...
        {selectedOrder ? <>
          ...
          <div className="staff-checkout-form">
            ...
            <button disabled={!cashSession || Number(paymentAmount || remaining) <= 0 || (paymentAllocationMode === 'seat' && paymentSeatNumbers.size === 0) || (paymentAllocationMode === 'item' && paymentItemIds.size === 0)} onClick={() => execute(() => { state.addRestaurantGuestOrderPayment(selectedOrder.id, Number(paymentAmount || remaining), paymentMethod, employee.name, cashSession?.id, { seatNumbers: [...paymentSeatNumbers], itemIds: [...paymentItemIds], tipAmount: Number(paymentTip) || 0, reference: paymentReference, source: 'terminal', operatorId: employee.id }); setPaymentSeatNumbers(new Set()); setPaymentItemIds(new Set()); setPaymentTip('0'); setPaymentReference(''); setPaymentAmount(''); }, `Paiement ${PAYMENT_LABELS[paymentMethod]} enregistré et rattaché à la caisse.`)}><HandCoins size={18} /> Valider {formatFCFA(Number(paymentAmount || remaining))}</button>
          </div>
          <div className="staff-checkout-sensitive"><button className="staff-approval-request" onClick={() => execute(() => state.requestEmployeeApproval({ type: 'discount', referenceId: selectedOrder.id, requestedBy: employee.id, requestedByName: employee.name, label: `Remise table ${selectedOrder.tableNumber || selectedOrder.id}`, reason: 'Geste commercial demandé par le client.', amount: Math.round(selectedOrder.total * 0.1), metadata: { orderId: selectedOrder.id } }), 'Demande de remise envoyée au manager.')}><ShieldCheck size={16} /> Remise 10 %</button><button className="staff-approval-request" onClick={() => execute(() => state.requestEmployeeApproval({ type: 'complimentary', referenceId: selectedOrder.id, requestedBy: employee.id, requestedByName: employee.name, label: `Offert table ${selectedOrder.tableNumber || selectedOrder.id}`, reason: 'Attention commerciale soumise à validation.', amount: Math.min(2000, selectedOrder.total), metadata: { orderId: selectedOrder.id } }), 'Demande d’offert envoyée au manager.')}><ShieldCheck size={16} /> Offert 2 000</button><button className="staff-approval-request" onClick={() => execute(() => state.requestEmployeeApproval({ type: 'cash_difference', referenceId: cashSession?.id || selectedOrder.id, requestedBy: employee.id, requestedByName: employee.name, label: `Incident terminal · table ${selectedOrder.tableNumber || selectedOrder.id}`, reason: `Transaction ${PAYMENT_LABELS[paymentMethod]} à rapprocher${paymentReference ? ` · ${paymentReference}` : ''}.`, amount: Number(paymentAmount || remaining), metadata: { orderId: selectedOrder.id, paymentMethod, reference: paymentReference || 'sans référence' } }), 'Incident terminal transmis au manager sans créer de paiement en double.')}><AlertCircle size={16} /> Incident terminal</button></div>
  ...
  const renderReceptionAction = () => {
    ...
    const selectedBalance = selectedFolio ? selectedFolio.charges.reduce((sum, item) => sum + item.amount, 0) - selectedFolio.payments.reduce((sum, item) => sum + item.amount, 0) : 0;
    ...
    })}</div>{selectedReservation ? <aside className="staff-reception-detail"><header><span>{selectedReservation.confirmationNumber}</span><h3>{selectedGuest?.fullName}</h3><p>{selectedReservation.adults + selectedReservation.children} personne(s) · départ {selectedReservation.departureDate}</p></header><div className="staff-reception-checks"><span className={selectedReservation.guaranteeStatus === 'secured' ? 'ok' : 'alert'}>{selectedReservation.guaranteeStatus === 'secured' ? <CheckCircle2 size={15} /> : <AlertCircle size={15} />} Garantie {selectedReservation.guaranteeStatus === 'secured' ? 'sécurisée' : 'à contrôler'}</span><span className={selectedRoom && ['clean', 'inspected'].includes(selectedRoom.housekeepingStatus) ? 'ok' : 'alert'}>{selectedRoom && ['clean', 'inspected'].includes(selectedRoom.housekeepingStatus) ? <CheckCircle2 size={15} /> : <AlertCircle size={15} />} {selectedRoom ? `Chambre ${selectedRoom.roomNumber} · ${selectedRoom.housekeepingStatus}` : 'Chambre non attribuée'}</span><span className={selectedFolio ? 'ok' : 'alert'}>{selectedFolio ? <CheckCircle2 size={15} /> : <AlertCircle size={15} />} Folio {selectedFolio ? 'ouvert' : 'à créer au check-in'}</span></div>{!selectedRoom && <section><h4>Chambres prêtes</h4><div className="staff-room-options">{availableRooms.map(room => <button key={room.id} onClick={() => execute(() => state.assignPMSRoom(selectedReservation.id, room.id, true), `Chambre ${room.roomNumber} attribuée.`)}><strong>{room.roomNumber}</strong><span>{room.roomType}</span><small>{formatFCFA(room.nightlyRate)}</small></button>)}</div></section>}{selectedRoom && selectedReservation.status === 'confirmed' && <button className="staff-primary-action" disabled={selectedReservation.guaranteeStatus !== 'secured' || !['clean', 'inspected'].includes(selectedRoom.housekeepingStatus)} onClick={() => execute(() => state.completePMSCheckIn(selectedReservation.id, { identity: true, guarantee: true, payment: true, signature: true, keyIssued: true }), `Check-in terminé · chambre ${selectedRoom.roomNumber}.`)}><DoorOpen size={17} /> Finaliser le check-in</button>}{selectedReservation.status === 'checked_in' && <section className="staff-stay-checkout"><div className="staff-stay-active"><CheckCircle2 size={21} /><span><strong>Séjour actif</strong><small>{selectedRoom ? `Clé chambre ${selectedRoom.roomNumber}` : 'Chambre à vérifier'} · solde {formatFCFA(Math.max(0, selectedBalance))}</small></span></div>{selectedFolio && selectedBalance > 0 && canEmployee('folio_payment') && <div className="staff-reception-payment"><label>Montant<input inputMode="numeric" value={receptionPaymentAmount || String(Math.max(0, selectedBalance))} onChange={event => setReceptionPaymentAmount(event.target.value.replace(/\D/g, ''))} /></label><label>Paiement<select value={receptionPaymentMethod} onChange={event => setReceptionPaymentMethod(event.target.value as PaymentType)}>{(['wave', 'orange_money', 'cash', 'card', 'other'] as PaymentType[]).map(method => <option key={method} value={method}>{PAYMENT_LABELS[method]}</option>)}</select></label><label>Référence<input value={receptionPaymentReference} onChange={event => setReceptionPaymentReference(event.target.value)} placeholder="Transaction ou reçu" /></label><button onClick={() => execute(() => { state.addPMSFolioPayment(selectedFolio.id, Math.min(selectedBalance, Number(receptionPaymentAmount || selectedBalance)), receptionPaymentMethod, receptionPaymentReference || `${employee.name} · Réception`); setReceptionPaymentAmount(''); setReceptionPaymentReference(''); }, `Paiement ${PAYMENT_LABELS[receptionPaymentMethod]} enregistré sur le folio.`)}><HandCoins size={16} /> Encaisser</button></div>}{selectedFolio && selectedBalance <= 0 && <button className="staff-primary-action" onClick={() => execute(() => state.updatePMSReservationStatus(selectedReservation.id, 'checked_out'), `Départ terminé · chambre ${selectedRoom?.roomNumber} transmise aux étages.`)}><LogOut size={17} /> Finaliser le départ</button>}</section>}</aside> : <aside className="staff-room-legend"><h3>État des chambres</h3>{[['vacant', 'Libre'], ['occupied', 'Occupée'], ['dirty', 'À nettoyer'], ['maintenance', 'Maintenance']].map(([status, label]) => <div key={status}><i className={status} /><span>{label}</span><strong>{status === 'dirty' ? db.pmsRooms.filter(item => item.housekeepingStatus === 'dirty').length : db.pmsRooms.filter(item => item.status === status).length}</strong></div>)}<button onClick={() => setReceptionWorkspace('rooms')}><BedDouble size={16} /> Voir toutes les chambres</button></aside>}</section>}
    </section>;
  };

  const renderHousekeeperAction = () => (
    <section className="staff-housekeeping"><header><div><span>{employee.role === 'housekeeping_manager' ? 'CONTRÔLE QUALITÉ' : 'ÉTAGES'}</span><h2>{employee.role === 'housekeeping_manager' ? 'Chambres à valider' : 'Mes chambres par priorité'}</h2><p>{employee.role === 'housekeeping_manager' ? 'La gouvernante remet une chambre en vente uniquement après son contrôle final.' : 'Nettoyage, linge, minibar, salle de bain et preuve photo avant transmission.'}</p></div><div className="staff-room-legend-inline"><span><i className="dirty" /> À faire</span><span><i className="in_progress" /> En cours</span><span><i className="clean" /> Terminée</span><span><i className="inspected" /> Contrôlée</span></div></header><div className="staff-room-task-grid">{db.pmsHousekeepingTasks.filter(task => employee.role === 'housekeeping_manager' ? ['completed', 'inspected'].includes(task.status) : !['completed', 'inspected'].includes(task.status)).slice().sort((a, b) => Number(b.priority === 'urgent') - Number(a.priority === 'urgent') || (db.pmsRooms.find(item => item.id === a.roomId)?.roomNumber || '').localeCompare(db.pmsRooms.find(item => item.id === b.roomId)?.roomNumber || '')).map(task => {
      const room = db.pmsRooms.find(item => item.id === task.roomId);
      const nextStatus = employee.role === 'housekeeping_manager' ? 'inspected' : task.status === 'pending' ? 'in_progress' : 'completed';
      ...
      return <article className={`${task.status} ${task.priority}`} key={task.id}>
        ...
        {employee.role === 'housekeeper' && task.status !== 'pending' && <div className="staff-room-checklist">{requiredChecks.map(check => <button className={checks.includes(check) ? 'checked' : ''} key={check} onClick={() => toggleHousekeepingCheck(task.id, check)}>{checks.includes(check) ? <Check size={13} /> : <i />}{check === 'salle-de-bain' ? 'Salle de bain' : check.charAt(0).toUpperCase() + check.slice(1)}</button>)}</div>}
        {employee.role === 'housekeeping_manager' && <div className="staff-housekeeping-quality"><span className={task.linenStatus === 'complete' ? 'ok' : 'alert'}>{task.linenStatus === 'complete' ? <CheckCircle2 size={14} /> : <AlertCircle size={14} />} Linge</span><span className={task.minibarStatus === 'checked' ? 'ok' : 'alert'}>{task.minibarStatus === 'checked' ? <CheckCircle2 size={14} /> : <AlertCircle size={14} />} Minibar</span><span className={(task.photoCount || 0) > 0 ? 'ok' : 'alert'}><Camera size={14} /> {task.photoCount || 0} preuve(s)</span></div>}
        <footer>{task.status !== 'inspected' && <button disabled={(nextStatus === 'completed' && !checklistComplete) || (employee.role === 'housekeeping_manager' && !canEmployee('housekeeping_validation'))} onClick={() => execute(() => state.updatePMSHousekeepingTask(task.id, nextStatus, employee.name), `Chambre ${room?.roomNumber} : ${ORDER_STATUS[nextStatus].toLowerCase()}.`)}>{employee.role === 'housekeeping_manager' ? 'Valider et remettre en vente' : nextStatus === 'in_progress' ? 'Démarrer' : checklistComplete ? 'Terminer et transmettre' : `${checks.length}/4 contrôles`}</button>}{employee.role === 'housekeeping_manager' && task.status === 'completed' ? <button className="staff-photo-action" onClick={() => execute(() => state.updatePMSHousekeepingTask(task.id, 'in_progress', `${employee.name} · reprise demandée`), `Chambre ${room?.roomNumber} renvoyée à l’équipe avec contrôle à reprendre.`)}><XCircle size={15} /> À reprendre</button> : <button className="staff-photo-action" onClick={() => execute(() => { state.updatePMSHousekeepingDetails(task.id, { photoCount: (task.photoCount || 0) + 1 }, employee.name); if (!checks.includes('photo')) toggleHousekeepingCheck(task.id, 'photo'); }, `Photo ajoutée à la chambre ${room?.roomNumber}.`)}><Camera size={15} /> Photo</button>}</footer>
      </article>;
    })}</div></section>
  );
  ...
  const renderDispatcherAction = () => {
    const drivers = db.employeeProfiles.filter(item => item.active && item.role === 'driver' && item.siteId === employee.siteId);
    const orders = db.deliveryOrders.filter(item => ['ready', 'out_for_delivery', 'failed'].includes(item.status)).sort((a, b) => Number(a.status !== 'failed') - Number(b.status !== 'failed') || a.updatedAt.localeCompare(b.updatedAt));
    const selected = orders.find(item => item.id === selectedDeliveryId) || orders[0];
    const selectedDriver = drivers.find(item => item.id === selectedDispatchDriverId) || drivers.find(item => item.name === selected?.driverName) || drivers[0];
    return <section className="staff-dispatch-layout"><div className="staff-dispatch-map"><header><Route size={21} /><div><span>TOURNÉES EN DIRECT</span><h2>Promesse, zone et livreur dans la même décision</h2><p>Les commandes en incident restent en tête jusqu’à leur résolution.</p></div></header><div className="staff-dispatch-zones">{Array.from(new Set(orders.map(item => item.zone || 'Hors zone'))).map(zone => { const zoneOrders = orders.filter(item => (item.zone || 'Hors zone') === zone); return <article key={zone}><MapPinned size={19} /><span><strong>{zone}</strong><small>{zoneOrders.length} course(s) · {zoneOrders.filter(item => item.status === 'failed').length} incident(s)</small></span><b>{zoneOrders.filter(item => item.status === 'out_for_delivery').length} en route</b></article>; })}</div><div className="staff-dispatch-orders">{orders.map(order => <button className={`${order.status} ${selected?.id === order.id ? 'active' : ''}`} key={order.id} onClick={() => { setSelectedDeliveryId(order.id); const driver = drivers.find(item => item.name === order.driverName); setSelectedDispatchDriverId(driver?.id || ''); }}><span><strong>{order.id}</strong><small>{order.customerName} · {order.zone || order.address}</small></span><b>{order.status === 'failed' ? 'INCIDENT' : ORDER_STATUS[order.status]}</b><em>{order.driverName || 'Sans livreur'}</em></button>)}</div></div>{selected ? <aside className="staff-dispatch-detail"><header><span>{selected.id}</span><h3>{selected.customerName}</h3><p>{selected.address} · {selected.deliverySlotLabel || 'Dès que possible'}</p></header>{selected.status === 'failed' && <div className="staff-delivery-alert"><AlertCircle size={20} /><span><strong>Incident à arbitrer</strong><small>{selected.deliveryIssue || 'Cause à confirmer avec le livreur'}</small></span></div>}<dl><div><dt>Encaissement</dt><dd>{selected.paymentStatus === 'paid' ? 'Déjà payé' : `${formatFCFA(selected.items.reduce((sum, line) => sum + line.quantity * line.salePrice, 0) + selected.deliveryFee)} à sécuriser`}</dd></div><div><dt>Repère</dt><dd>{selected.landmark || 'À confirmer'}</dd></div><div><dt>Promesse</dt><dd>{selected.estimatedMinutes || 45} min</dd></div></dl><label>Livreur<select value={selectedDriver?.id || ''} onChange={event => setSelectedDispatchDriverId(event.target.value)}>{drivers.map(driver => <option key={driver.id} value={driver.id}>{driver.name} · {db.employeeShifts.some(shift => shift.employeeId === driver.id && shift.status === 'open') ? 'en service' : 'disponible'}</option>)}</select></label>{selected.status === 'ready' && <button className="staff-primary-action" disabled={!selectedDriver || !canEmployee('delivery_dispatch')} onClick={() => execute(() => { state.assignDeliveryDriver(selected.id, selectedDriver!.name, selectedDriver!.phone, employee.name); const result = state.dispatchDeliveryOrder(selected.id, { id: employee.id, name: employee.name }); if (!result.success) throw new Error(result.error); }, `${selected.id} affectée à ${selectedDriver?.name} et lancée en tournée.`)}><Truck size={18} /> Affecter et lancer</button>}{selected.status === 'out_for_delivery' && <button className="staff-primary-action" disabled={!selectedDriver || selectedDriver.name === selected.driverName} onClick={() => execute(() => state.assignDeliveryDriver(selected.id, selectedDriver!.name, selectedDriver!.phone, employee.name), `Course réaffectée à ${selectedDriver?.name}.`)}><UsersRound size={18} /> Réaffecter la course</button>}{selected.status === 'failed' && <button className="staff-delivery-incident" onClick={() => execute(() => state.returnDeliveryOrder(selected.id, { id: employee.id, name: employee.name }), 'Retour au dépôt confirmé et stock orienté vers le contrôle.')}>Organiser le retour dépôt</button>}</aside> : <div className="staff-list-empty"><CheckCircle2 size={30} /><strong>Aucune tournée à piloter</strong></div>}</section>;
  };
  ...
  const renderMaintenanceAction = () => {
    const tickets = db.pmsMaintenanceTickets.slice().sort((a, b) => Number(b.priority === 'critical') - Number(a.priority === 'critical') || Number(b.priority === 'urgent') - Number(a.priority === 'urgent') || b.openedAt.localeCompare(a.openedAt));
    const selected = tickets.find(item => item.id === selectedMaintenanceId) || tickets.find(item => item.status === 'in_progress') || tickets.find(item => item.status === 'open') || tickets[0];
    const room = db.pmsRooms.find(item => item.id === selected?.roomId);
    return <section className="staff-maintenance-layout"><div className="staff-maintenance-queue"><header><Wrench size={21} /><div><span>INTERVENTIONS</span><h2>Priorité à la chambre immobilisée</h2><p>Chaque étape conserve l’auteur, le coût et les preuves.</p></div></header>{tickets.map(ticket => { const ticketRoom = db.pmsRooms.find(item => item.id === ticket.roomId); return <button className={`${ticket.priority} ${ticket.status} ${selected?.id === ticket.id ? 'active' : ''}`} key={ticket.id} onClick={() => { setSelectedMaintenanceId(ticket.id); setMaintenanceCost(String(ticket.actualCost || ticket.estimatedCost || '')); setMaintenanceNote(ticket.note); }}><span><strong>Chambre {ticketRoom?.roomNumber || ticket.roomId}</strong><small>{ticket.equipment} · {ticket.assignedTo}</small></span><b>{ticket.priority === 'critical' ? 'CRITIQUE' : ticket.priority === 'urgent' ? 'URGENT' : ORDER_STATUS[ticket.status] || ticket.status}</b><em>{ticket.actualCost ? formatFCFA(ticket.actualCost) : `Estimé ${formatFCFA(ticket.estimatedCost)}`}</em></button>; })}</div>{selected ? <aside className="staff-maintenance-detail"><header><span>{selected.priority.toUpperCase()}</span><h3>{selected.equipment}</h3><p>Chambre {room?.roomNumber || selected.roomId} · ouverte {new Date(selected.openedAt).toLocaleString('fr-FR')}</p></header><div className="staff-maintenance-status"><i className={selected.status} /><span><strong>{ORDER_STATUS[selected.status] || selected.status}</strong><small>{selected.unavailableUntil ? `Indisponible jusqu’au ${new Date(selected.unavailableUntil).toLocaleString('fr-FR')}` : 'Remise en service à confirmer'}</small></span></div><label>Compte rendu<textarea value={selected.id === selectedMaintenanceId ? maintenanceNote : selected.note} onChange={event => { setSelectedMaintenanceId(selected.id); setMaintenanceNote(event.target.value); }} /></label><label>Coût réel (FCFA)<input inputMode="numeric" value={selected.id === selectedMaintenanceId ? maintenanceCost : String(selected.actualCost || selected.estimatedCost)} onChange={event => { setSelectedMaintenanceId(selected.id); setMaintenanceCost(event.target.value.replace(/\D/g, '')); }} /></label><div className="staff-maintenance-proof"><Camera size={18} /><span><strong>{selected.photoCount || 0} preuve(s)</strong><small>Diagnostic et état après intervention</small></span><button onClick={() => execute(() => state.updatePMSMaintenanceDetails(selected.id, { photoCount: (selected.photoCount || 0) + 1 }), 'Photo d’intervention ajoutée.')}>Ajouter</button></div>{selected.status === 'open' && <button className="staff-primary-action" disabled={!canEmployee('maintenance_update')} onClick={() => execute(() => { state.updatePMSMaintenanceDetails(selected.id, { assignedTo: employee.name, note: maintenanceNote || selected.note }); state.updatePMSMaintenanceTicket(selected.id, 'in_progress'); }, 'Intervention démarrée et chambre maintenue hors vente.') }><Wrench size={18} /> Démarrer l’intervention</button>}{selected.status === 'in_progress' && <button className="staff-primary-action" disabled={!maintenanceNote.trim() || !canEmployee('maintenance_update')} onClick={() => execute(() => { state.updatePMSMaintenanceDetails(selected.id, { assignedTo: employee.name, actualCost: Number(maintenanceCost) || 0, note: maintenanceNote }); state.updatePMSMaintenanceTicket(selected.id, 'resolved'); }, 'Intervention résolue et envoyée au contrôle final.') }><CheckCircle2 size={18} /> Résoudre et transmettre</button>}{selected.status === 'resolved' && <div className="staff-maintenance-wait"><ShieldCheck size={19} /><span><strong>Contrôle indépendant attendu</strong><small>La chambre ne revient en vente qu’après vérification.</small></span></div>}</aside> : <div className="staff-list-empty"><CheckCircle2 size={30} /><strong>Aucun ticket de maintenance</strong></div>}</section>;
  };
  ...
  const renderDispatcherGameChanger = () => {
    const ready = db.deliveryOrders.filter(item => item.status === 'ready');
    const active = db.deliveryOrders.filter(item => item.status === 'out_for_delivery');
    const incidents = db.deliveryOrders.filter(item => item.status === 'failed');
    const drivers = db.employeeProfiles.filter(item => item.active && item.role === 'driver' && item.siteId === employee.siteId);
    const driversOnDuty = drivers.filter(driver => db.employeeShifts.some(shift => shift.employeeId === driver.id && shift.status === 'open'));
    return <section className="staff-game-changer staff-dispatch-intelligence"><header><CircleGauge size={22} /><div><span>TOUR DE CONTRÔLE LIVRAISON</span><h2>Une promesse claire pour chaque commande</h2></div><b className={incidents.length ? 'danger' : 'good'}>{incidents.length} incident(s)</b></header><div className="staff-intelligence-metrics"><article><small>Prêtes à partir</small><strong>{ready.length}</strong><p>{ready.filter(item => !item.driverName).length} sans livreur affecté</p></article><article><small>Courses actives</small><strong>{active.length}</strong><p>{active.filter(item => item.paymentStatus !== 'paid').length} encaissement(s) à sécuriser</p></article><article><small>Livreurs disponibles</small><strong>{driversOnDuty.length}/{drivers.length}</strong><p>Présence issue des prises de service</p></article><article><small>Promesse sous tension</small><strong>{[...ready, ...active].filter(item => elapsedMinutes(item.createdAt) > (item.estimatedMinutes || 45)).length}</strong><p>Commandes au-delà du délai annoncé</p></article></div></section>;
  };

  const renderMaintenanceGameChanger = () => {
    const open = db.pmsMaintenanceTickets.filter(item => !['resolved', 'verified'].includes(item.status));
    const critical = open.filter(item => item.priority === 'critical');
    const unavailableRooms = db.pmsRooms.filter(item => item.status === 'maintenance');
    const estimated = open.reduce((sum, item) => sum + item.estimatedCost, 0);
    return <section className="staff-game-changer staff-maintenance-intelligence"><header><Wrench size={22} /><div><span>DISPONIBILITÉ DES CHAMBRES</span><h2>Réduire chaque minute d’immobilisation</h2></div><b className={critical.length ? 'danger' : 'good'}>{critical.length} critique(s)</b></header><div className="staff-intelligence-metrics"><article><small>Chambres indisponibles</small><strong>{unavailableRooms.length}</strong><p>{unavailableRooms.map(room => room.roomNumber).join(', ') || 'Aucune chambre immobilisée'}</p></article><article><small>Interventions ouvertes</small><strong>{open.length}</strong><p>{open.filter(item => item.status === 'in_progress').length} en cours</p></article><article><small>Budget estimé</small><strong>{formatFCFA(estimated)}</strong><p>À rapprocher des coûts réels</p></article><article><small>Preuves disponibles</small><strong>{open.reduce((sum, item) => sum + (item.photoCount || 0), 0)}</strong><p>Photos attachées aux diagnostics</p></article></div></section>;
  };
  ...
  const renderRoleGameChanger = () => {
    switch (employee.role) {
      ...
      case 'housekeeping_manager': return renderHousekeeperGameChanger();
      case 'storekeeper': return renderStorekeeperGameChanger();
      case 'picker': return renderPickerGameChanger();
      case 'dispatcher': return renderDispatcherGameChanger();
      case 'driver': return renderDriverGameChanger();
      case 'maintenance': return renderMaintenanceGameChanger();
  ...
  const renderRoleAction = () => {
    ...
    switch (employee.role) {
      ...
      case 'housekeeping_manager': workspace = renderHousekeeperAction(); break;
      case 'storekeeper': workspace = renderStorekeeperAction(); break;
      case 'picker': workspace = renderPickerAction(); break;
      case 'dispatcher': workspace = renderDispatcherAction(); break;
      case 'driver': workspace = renderDriverAction(); break;
      case 'maintenance': workspace = renderMaintenanceAction(); break;
  ...
  const renderMore = (directSchedule = false) => {
    ...
    const activeLifeSection: EmployeeLifeSection = directSchedule ? 'schedule' : lifeSection;

    return <section className="staff-life-shell">
      <header className="staff-life-heading"><div className="staff-avatar"><RoleIcon size={24} /></div><div><span>{directSchedule ? 'ORGANISER MA SEMAINE' : 'MON ESPACE'}</span><h1>{directSchedule ? 'Mon planning' : employee.name}</h1><p>{directSchedule ? `${employee.name} · horaires, affectations et demandes de changement.` : `${roleConfig.label} · un espace personnel, utile avant, pendant et après le service.`}</p></div>{directSchedule ? <button onClick={() => setTab('more')}><UserRoundSearch size={16} /> Mon espace</button> : !demoAutoStart && <button onClick={() => { setLoggedIn(false); setPin(''); }}><LogOut size={16} /> Verrouiller</button>}</header>
      {!directSchedule && <nav className="staff-life-nav">{lifeNav.map(item => { const Icon = item.icon; return <button className={activeLifeSection === item.id ? 'active' : ''} key={item.id} onClick={() => setLifeSection(item.id)}><Icon size={17} /><span>{item.label}</span></button>; })}</nav>}

      {activeLifeSection === 'overview' && <div className="staff-life-grid">
      ...
      {activeLifeSection === 'support' && <div className="staff-support-space">
      ...
      {activeLifeSection === 'handover' && <div className="staff-handover-space">
  ...
  const tabs: Array<{ id: StaffTab; label: string; mobileLabel: string; icon: LucideIcon }> = [
    ...
    { id: 'schedule', label: preferences.language === 'wo' ? 'Waxtu yi' : 'Planning', mobileLabel: preferences.language === 'wo' ? 'Waxtu' : 'Planning', icon: CalendarCheck }
  ];
  ...
  return <section className={`employee-workspace staff-app ${preferences.highContrast ? 'staff-high-contrast' : ''} ${preferences.lowBandwidth ? 'staff-low-bandwidth' : ''}`} style={{ '--staff-role': roleConfig.color, '--staff-brand': siteBrand?.primaryColor || db.sartalBrandSettings.primaryColor } as React.CSSProperties}>
    ...
    <main className="staff-app-content">{tab === 'today' && renderToday()}{tab === 'tasks' && renderTasks()}{tab === 'action' && renderRoleAction()}{tab === 'messages' && renderMessages()}{tab === 'schedule' && renderMore(true)}{tab === 'more' && renderMore()}</main>
