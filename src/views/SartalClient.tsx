  ...
  return (
    ...
    >
      <header className="sartal-client-header">
        ...
        {!standalone && <div className="client-preview-controls"><select className="form-control" aria-label="Client affiché dans l’aperçu" value={customer.id} onChange={event => changeCustomer(event.target.value)}>{db.sartalCustomers.map(item => <option value={item.id} key={item.id}>{item.fullName}</option>)}</select><button className="btn btn-primary" title="Ouvre le parcours public séparé, avec identification du client" onClick={() => window.open(`${window.location.origin}${window.location.pathname}?client=${mode}`, '_blank', 'noopener,noreferrer')}>Tester l’accès public <ArrowRight size={16} /></button></div>}
      </header>
