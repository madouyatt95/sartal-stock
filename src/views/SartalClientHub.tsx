...
export const SartalClientHub: React.FC<SartalClientHubProps> = ({ state, customerId, onOpenRestaurant, onOpenDelivery, onMessage }) => {
  ...
  const deliveryOrder = deliveryEnabled ? db.deliveryOrders.find(item => item.customerId === customerId && !['delivered', 'returned', 'cancelled'].includes(item.status)) || db.deliveryOrders.find(item => item.customerId === customerId) : undefined;
  const pmsGuest = pmsEnabled ? db.pmsGuests.find(item => item.phone.replace(/\s/g, '') === customer?.phone.replace(/\s/g, '')) : undefined;
  const pmsReservation = pmsEnabled ? db.pmsReservations.find(item => item.guestId === pmsGuest?.id && ['confirmed', 'checked_in'].includes(item.status)) : undefined;
  ...
  const serviceRequests = db.sartalServiceRequests.filter(item => item.customerId === customerId && !['completed', 'cancelled'].includes(item.status) && (item.context === 'restaurant' ? restaurantEnabled : item.context === 'delivery' ? deliveryEnabled : pmsEnabled));
  const loyalty = db.sartalLoyaltyTransactions.filter(item => item.customerId === customerId).sort((a, b) => b.date.localeCompare(a.date));
  const feedback = db.sartalCustomerFeedback.filter(item => item.customerId === customerId && (item.context === 'restaurant' ? restaurantEnabled : deliveryEnabled)).sort((a, b) => b.submittedAt.localeCompare(a.submittedAt));
  const journeyItems = db.sartalJourneyItems.filter(item => item.customerId === customerId && item.status !== 'cancelled' && (item.context === 'restaurant' ? restaurantEnabled : item.context === 'delivery' ? deliveryEnabled : pmsEnabled)).sort((a, b) => a.scheduledAt.localeCompare(b.scheduledAt));
  const favoriteProducts = db.products.filter(item => customer?.favoriteProductIds?.includes(item.id));
  const recurringOrder = deliveryEnabled ? db.sartalRecurringOrders.find(item => item.customerId === customerId) : undefined;
  ...
  const timeline = useMemo(() => {
    ...
    if (restaurantEnabled) db.restaurantReservations.filter(item => item.customerId === customerId).forEach(item => events.push({ id: item.id, date: item.createdAt, icon: <CalendarCheck size={17} />, title: 'Table réservée', detail: `${item.guests} personne(s) · ${item.time}`, tone: 'restaurant' }));
    if (restaurantEnabled) db.restaurantGuestOrders.filter(item => item.customerId === customerId).forEach(item => events.push({ id: item.id, date: item.createdAt, icon: <UtensilsCrossed size={17} />, title: 'Commande restaurant', detail: `${formatFCFA(item.total)} · ${item.status}`, tone: 'restaurant' }));
    if (deliveryEnabled) db.deliveryOrders.filter(item => item.customerId === customerId).forEach(item => events.push({ id: item.id, date: item.createdAt, icon: <Truck size={17} />, title: 'Commande livraison', detail: `${item.id} · ${item.status}`, tone: 'delivery' }));
  ...
  }, [customerId, db, deliveryEnabled, feedback, loyalty, restaurantEnabled]);
  ...
  const askReception = () => {
    ...
    requestSartalService({ customerId: customer.id, context, referenceId: pmsReservation?.id || restaurantOrder?.id || deliveryOrder?.id, type: pmsReservation ? 'reception' : restaurantEnabled ? 'waiter' : 'delivery_help', label: pmsReservation ? 'Contacter la réception' : restaurantEnabled ? 'Contacter le responsable de salle' : 'Être rappelé par le service livraison', priority: 'urgent' });
  ...
  const rewards = [
    { points: 500, label: 'Livraison offerte', enabled: deliveryEnabled },
    { points: 1000, label: 'Attention restaurant de 5 000 FCFA', enabled: restaurantEnabled },
    { points: 1500, label: 'Départ tardif selon disponibilité', enabled: pmsEnabled }
  ...
  return <section className="sartal-hub">
    ...
    {tab === 'today' && <div className="sartal-hub-today">
      <section className="sartal-now-card">
        <div><span>VOTRE PROCHAINE ACTION</span><h2>{restaurantOrder ? `Suivre la table ${restaurantOrder.tableNumber || ''}` : deliveryOrder ? 'Suivre votre livraison' : pmsReservation ? `Profiter de la chambre ${pmsRoom?.roomNumber || ''}` : restaurantEnabled ? 'Découvrir le restaurant' : 'Découvrir la boutique'}</h2><p>{restaurantOrder ? 'La cuisine et votre serveur partagent le même suivi.' : deliveryOrder ? 'Votre commande et votre livreur sont visibles en temps réel.' : pmsReservation ? 'Réception, folio et services sont déjà reliés.' : restaurantEnabled && deliveryEnabled ? 'Restaurant et livraison sont accessibles avec le même profil.' : restaurantEnabled ? 'Réservation, carte et service sont réunis ici.' : 'Catalogue, livraison et suivi sont réunis ici.'}</p></div>
        <button onClick={restaurantOrder ? onOpenRestaurant : deliveryOrder ? onOpenDelivery : pmsReservation ? askReception : restaurantEnabled ? onOpenRestaurant : onOpenDelivery}>Continuer <ArrowRight size={17} /></button>
      </section>
